...
class TestCaseVideoShareVideoListByGameId(HttpRunner):  #更改为与接口相关的名字，方便以后引用（例如引用login接口需要先导入类名）
    ...
    config = (
        Config("share_video_list_by_game_id")
        .variables(
        **{
    ...
    teststeps = [
        ...
        Step(
            RunRequest("share_video_list_by_game_id")
            .with_variables(
                **{   
                }
                )
...
if __name__ == "__main__":
    TestCaseVideoShareVideoListByGameId().test_start()