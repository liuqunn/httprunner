...
class TestCaseGameListGetGameCommentByDetail(HttpRunner):  #更改为与接口相关的名字，方便以后引用（例如引用login接口需要先导入类名）
    ...
    config = (
        Config("get_game_comment_by_detail")
        .variables(
        **{
    ...
    teststeps = [
        ...
        Step(
            RunRequest("get_game_comment_by_detail")
            .with_variables(
                **{   
                }
                )
...
if __name__ == "__main__":
    TestCaseGameListGetGameCommentByDetail().test_start()